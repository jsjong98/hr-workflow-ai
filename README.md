...
> 터미널이나 명령어 없이 파일 더블클릭 한 번으로 실행하는 방법입니다.

### 1단계: Node.js 설치 (최초 1회만)

👉 **https://nodejs.org/ko** 접속 → **LTS 버전** 다운로드 → 설치 (모든 옵션 기본값 OK)

> Node.js는 이 앱을 실행하기 위한 필수 프로그램입니다. 한 번만 설치하면 됩니다.

---

### 2단계: 소스코드 다운로드

GitHub 페이지 상단 **Code** 버튼 → **Download ZIP** 클릭 → 압축 해제
...
| 운영체제 | 실행 파일 | 방법 |
|---------|----------|------|
| 🍎 **Mac** | `시작.command` | **우클릭 → "열기" → "열기" 클릭** ⚠️ 처음 한 번만 우클릭 필요 |
| 🪟 **Windows** | `start.bat` | **더블클릭** → "추가 정보" → "실행" 클릭 |

> **Mac 주의**: 처음 실행 시 "확인되지 않은 개발자" 경고가 뜹니다.  
> → **우클릭 → "열기"** 로 실행하면 됩니다. (두 번째부터는 더블클릭 OK)

> **Windows 주의**: "Windows가 PC를 보호했습니다" 창이 뜨면  
> → **"추가 정보" 클릭 → "실행" 클릭**
...
- 자동으로 설치 진행 (최초 1회, 약 2~3분)
- 완료되면 **브라우저가 자동으로 열리며** `http://localhost:3000` 접속됨
- 종료: 터미널 창 닫기 또는 `Ctrl + C`
